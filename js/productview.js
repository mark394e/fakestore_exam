...
async function footerCategoryClickedFromSingleview() {
  window.scrollTo(0, 0);
  try {
    const getCategoryProducts = await fetch(`https://fakestoreapi.com/products/category/${clickedCategoryFromSingleviewFooter}`);
    ...
    informUserOfError('Sorry! It seems like there is a problem with getting the products. Please try again');
  } finally {
...
function footerCategoryLinksClickEvent() {
  footerCategoryLinks.forEach((categoryLink) => categoryLink.addEventListener('click', filterProductsByCategory));
}
...
function filterButtonClickEvents() {
  theActualFilterButtons.forEach((filterButton) => filterButton.addEventListener('click', filterProductsByCategory));
...
async function filterProductsByCategory() {
  ...
  try {
    const getCategoryProducts = await fetch(`https://fakestoreapi.com/products/category/${this.dataset.category}`);
...
function showProducts(products) {
  ...
  products.forEach((product) => {
    ...
    // rest to lowercase
    let modifiedProductTitle = product.title.charAt(0).toUpperCase() + product.title.slice(1).toLowerCase();
    // Cloning
    ...
    const addToCartButton = cloneProductData.querySelector('button.add-to-cart-button');
    /* #### Placing the products content in our template fields #### */
    cloneProductData.querySelector('article.product').setAttribute('data-product-id', product.id);
    // Product img
    cloneProductData.querySelector('div.product-img-wrapper').style.backgroundImage = `url('${product.image}')`;
    // Product title (if longer than 10 chars we cut it)
    cloneProductData.querySelector('h2.product-title').textContent = modifiedProductTitle.length > 10 ? modifiedProductTitle.slice(0, 10) + '..' : modifiedProductTitle;
    // Accessibility on product title for screenreader, due to us cutting the length
    cloneProductData.querySelector('h2.product-title').setAttribute('aria-label', modifiedProductTitle);
    // Product price
    ...
    cloneProductData.querySelector('div.clickable-container').addEventListener('click', () => showSingleProduct(product));

    /* #### RELATED TO CART FUNCTIONALITY #### */
    addToCartButton.setAttribute('data-product-id', product.id);
    addToCartButton.addEventListener('click', (event) => addProductToCart(product, event));
